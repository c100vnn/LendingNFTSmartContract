    ...
    IBaseDoNFT
{
    ...
    function isValidNow(uint256 tokenId) override public view returns (bool isValid) {
        DoNftInfo storage info = doNftMapping[tokenId];
    ...
    function getNonce(uint256 tokenId) override external view returns (uint64) {
    ...
    function mint(
        ...
        address to,
        address user
    ) override public onlyNow(start) nonReentrant returns (uint256 tid) {
    ...
    function setMaxDuration(uint64 v) override public onlyAdmin {
        maxDuration = v;
    }

    function getMaxDuration() override public view returns (uint64) {
        return maxDuration;
    }
    ...
    function isVNft(uint256 tokenId) override public view returns (bool) {
        if (tokenId == 0) return false;

        return oid2vid[doNftMapping[tokenId].oid] == tokenId;
    }

    function getOriginalNftAddress() override external view returns (address) {
        return oNftAddress;
    }

    function getOriginalNftId(uint256 tokenId) override external view returns (uint256) {
        DoNftInfo storage info = doNftMapping[tokenId];
        return info.oid;
    }

    function getVNftId(uint256 originalNftId) override public view returns (uint256) {
    ...
    function exists(uint256 tokenId) public view override virtual returns (bool) {
