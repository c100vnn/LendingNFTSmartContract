...
contract Royalty is IRoyalty {
    ...
    function claimRoyaltyAdmin() public override virtual {
    ...
    function getBeneficiary() override external view returns (address payable) {
        return beneficiary;
    }

    function setRoyaltyFee(uint256 fee_) public override virtual onlyRoyaltyAdmin {
        require(fee_ <= 10000, "fee exceeds 10pct");
        fee = fee_;
        emit SetRoyaltyFee(msg.sender, fee);
    }

    function getRoyaltyFee() override public  view returns (uint256) {
        return fee;
    }
}
